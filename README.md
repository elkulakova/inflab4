...
Создала docker image (образ). Для этого написала Dockerfile. Создала пустой текстовый документ, где указываем образ, на основе которого будет работать наш. Также сразу устанавливаем утилиту `ping` 
```
FROM ubuntu:latest
